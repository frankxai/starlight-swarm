...
This slice closes one part of [runtime activation authority issue #15](https://github.com/frankxai/starlight-swarm/issues/15): a trusted service can convert two signed, operation-bound approvals into a durable reservation, consume it, issue one short-lived `leased-not-started` lease, redeem its separately bound credential into `start-authorized-not-observed`, accept one authenticated runner/channel claim as `runner-claimed-not-started`, renew that claim with a rotating authenticated heartbeat credential, and record one server-attested process start as `runner-start-observed`. Every receipt remains explicitly undispatched. Start observation proves only that the exact authenticated runtime reported the bound process instance as started; it does not prove workload effects, completion, model calls, repository changes, customer outcomes or spend.
...
`claimRunnerStart` is an unwired online boundary. It presents the precommitted control credential, a unique claim-request ID, a distinct precommitted heartbeat credential and a separate precommitted start-observation credential, but runner identity is never accepted from request fields. An injected server-owned transport attestor derives runner, instance, runtime, host and channel-binding identities. The default attestor always refuses, so this repository cannot claim a runner unless an embedding supplies authenticated session evidence. Under the same database lock, the store revalidates the broker principal, signed runner/runtime/host binding, preparation, revocations, fresh host evidence and committed budget ledgers. One conditional transition persists only credential digests and creates `runner-claimed-not-started`; exact retries require the same request, credentials, principal and channel. Drift after acceptance becomes `stop-requested` with committed resources retained. The claim expiry is bounded by the runner access review, evidence-age ceiling and signed operation timeout.
...
- There is no runner adapter, dispatch, workload-effect observation, terminal/non-start evidence, remote stop confirmation, resource recovery, kill-switch integration or external side effect. Heartbeat acceptance proves only fresh authenticated channel presence; start observation proves only a bound process-start record. The strongest later database boundary is a function-only role; the current exact column-grant contract still assumes arbitrary SQL cannot escape the reviewed authority module.
...
9. An embedding may supply a server-owned transport attestor and present distinct heartbeat and start-observation credentials to accept one bound claim. It must treat `runner-claimed-not-started` as authenticated connection evidence, not execution evidence. This repository's default attestor refuses.
...
12. Do not connect this primitive to dispatch before terminal/non-start evidence, recovery behavior, usage reconciliation, token-unit budgets, independent security review and the issue #15 adversarial evidence exist.
...
Focused adversarial coverage includes signature and token forgery, caller broker forgery, database-role posture and excess-grant rejection, fresh/disabled principal evidence, identity/binding drift, idempotent consumption, leasing, redemption, runner claim, rotating heartbeat and process-start observation, reserved-to-committed ledger movement, quarantine, expiry reconciliation, migration, immutable aggregate windows, ledger corruption and tombstone retention. When `TEST_DATABASE_URL` is set, the suite also races independent PostgreSQL authorities for consumption, leasing, redemption, runner claims, competing heartbeats, competing start observations, heartbeat expiry, host/principal disablement, prepared cancellation, cancellation, revocation and shared budget windows.
