...
This slice closes one part of [runtime activation authority issue #15](https://github.com/frankxai/starlight-swarm/issues/15): a trusted service can convert two signed, operation-bound approvals into a durable reservation, consume it, issue one short-lived `leased-not-started` lease, redeem its separately bound credential into `start-authorized-not-observed`, and accept one authenticated runner/channel claim as `runner-claimed-not-started`. Every receipt remains explicitly undispatched. Neither redemption nor claim proves that a worker started, ran, called a model, changed a repository, or produced customer or spend evidence.
...
- There is no runner adapter, dispatch, execution observation, heartbeat acceptance, terminal evidence, recovery, kill-switch integration or external side effect. The heartbeat token is only precommitted for a later contract. The strongest later database boundary is a function-only role; the current exact column-grant contract still assumes arbitrary SQL cannot escape the reviewed authority module.
...
10. Do not connect this primitive to dispatch before heartbeat/terminal evidence, recovery behavior, usage reconciliation, token-unit budgets, independent security review and the issue #15 adversarial evidence exist.
...
Focused adversarial coverage includes signature and token forgery, caller broker forgery, database-role posture and excess-grant rejection, fresh/disabled principal evidence, identity/binding drift, idempotent consumption, leasing, redemption and runner claim, reserved-to-committed ledger movement, quarantine, migration, immutable aggregate windows, ledger corruption and tombstone retention. When `TEST_DATABASE_URL` is set, the suite also races independent PostgreSQL authorities for consumption, leasing, redemption, runner claims, principal disablement, cancellation, revocation and shared budget windows.
